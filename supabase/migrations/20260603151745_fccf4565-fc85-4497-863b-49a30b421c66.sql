...
DROP FUNCTION IF EXISTS public.create_crew_as_admin(text, text, uuid, text, text);
...
RETURNS TABLE(new_crew_id uuid, new_invite_code text)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.crews c WHERE c.slug = _clean_slug) THEN
  ...
  INSERT INTO public.user_roles (user_id, role, crew_id)
  VALUES (_uid, 'admin'::app_role, _new_id)
  ON CONFLICT (user_id, role, crew_id) DO NOTHING;

  INSERT INTO public.admin_permissions (user_id, crew_id, is_super_admin, created_by)
  VALUES (_uid, _new_id, true, _uid)
  ON CONFLICT (user_id, crew_id) DO UPDATE SET is_super_admin = true;
  ...
  FROM public.team_config tc
  WHERE tc.crew_id = _template
  ON CONFLICT DO NOTHING;
